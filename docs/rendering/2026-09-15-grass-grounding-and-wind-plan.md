...
- **Final review, F1: the understory's ground tint.** The review found the `UNDERSTORY` profile tinting toward black: its `groundTint` turns the tint on, so the shader reads the `foliage` attribute, but `forestMeshes.ts` uploaded only `matrix`, `fadeBands` and `groundGrad` for that bucket, leaving the attribute at Babylon's generic `(0, 0, 0, 1)` and mixing the base of every fern up to 60 % toward black. Ruled: write a real buffer for the bucket (the same four floats `writeFoliage` writes for ground cover, from the same palette and density), AND guard the shader so a draw with no tint data cannot mix at all — `has = step(1/255, max(vFoliage.rgb))` on the tint weight. An architectural test now holds the rule for every bucket: whichever ones declare the attribute must fill it.
- **Final review, F2: the mist drift on a weather change.** The offset was `MIST_DRIFT · speed · seconds` off the absolute clock, so a change of speed rescaled the whole history and the banks teleported — clear to rain at ten minutes moved every one about 97 m in a frame. Ruled: integrate instead. The shell keeps the drift distance and adds `MIST_DRIFT · speed · min(1, max(0, Δt))` per update, the clamp so a suspended tab resumes rather than lurches; `seconds` stays the raw clock the caller passes.
- **Final review, F3: the gust sample with no wildlife.** `ambient.setWind` samples the gust at the listener, but the listener was only ever placed through `wildlifeAudio`, which a world with `hasWildlife === false` never builds — so the wind bed read the gust at the world origin for the whole match. Ruled: `syncWind` places the listener itself, every frame, and the left-to-right-handed mirror moves into one exported `listenerToAudio` helper both call sites use.
- **Final review, F4: the sway and the instance scale.** `foliageHeight` is the MODEL bounding height while instances draw at their own scale (trees 2.8–5×, grass 0.74–1.49×) and the displacement is added in world space, so the lean was a fraction of model rather than drawn height. Ruled: the motion weight multiplies by `length(finalWorld[1].xyz)`, the instance's uniform scale, which makes the quoted 2.9 %-to-19.8 % tip lean true at every size.
