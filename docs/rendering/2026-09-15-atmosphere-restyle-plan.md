...
- Never mention how any asset was made, and never name the tools or the process behind the assets in code, comments, docs or commit messages. The repository's git pre-push hook runs a leak scan over the working tree and the unpushed commits; run the same command by hand (it is the one line in `.git/hooks/pre-push`) before every commit, and zero failing is the bar. It rejects the names of content-creation tools and the word for building models from photographs; say "scanned" if you must say anything.
