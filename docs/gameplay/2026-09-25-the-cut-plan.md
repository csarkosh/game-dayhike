...
- **No Hollow ever steps out on safe ground, or on a player.** A fork that itself stands on the road corridor is never cut (it is safe ground; nobody is hunted there). For a closed branch the Hollow steps out `FORK_SPAWN_DIST` (12 m) into it, or, when the branch enters the corridor sooner, `FORK_SPAWN_CLEAR` (1 m) short of where it does; and never within `FORK_SPAWN_PLAYER_CLEAR` (2 m) of any living player — the spawn steps further along the bed until it is, within the same limits. A branch with no such point in its first `FORK_SPAWN_MIN` (2 m) or beyond cannot be closed and stays open. A fork is recorded as cut once judged, whatever was closed: with its open neighbour, or `-1` when it stands on the corridor or nothing but the arrival reaches the pad.
- **A fork is never a trap at the moment it is cut.** The open branch always reaches the pad on the residual graph at cut time; if no branch but the arrival does, the fork is recorded as cut with nothing closed. That holds on the guide too (2026-09-25): the guide's edge out is opened only while the guide beyond it still reaches the pad on the residual graph, otherwise the fork is judged as a stray's — before that, on `hollow24` and `hollow44`, a guide fork cut early had closed the guide ahead and a later guide fork opened a way that led only to Hollows. Later cuts elsewhere can put a Hollow on that route (two players on different branches can arrange it), never remove the trail: a closed branch is walkable. The spec's sentence claiming more is reworded to this.
...
- Exact values from the spec: `FORK_CUT_RADIUS = 30` (moved to 9 on 2026-09-25, with a new `FORK_MOUTH_DIST = 3`: at thirty metres the fork Hollow met a sprinter head-on at the fork); `FORK_SPAWN_DIST = 12`; `FORK_REVEAL_S = 1`; `GUIDE_REJOIN_SLACK = 60`; `GUIDE_MIN`/`GUIDE_MAX`/`GUIDE_TRIES` as shipped (1.5, 2.5, 64). New here: `FORK_SPAWN_CLEAR = 1`, `FORK_SPAWN_MIN = 2`, `FORK_SPAWN_PLAYER_CLEAR = 2`, `FORK_EMERGE_MAX_S = 6`.
