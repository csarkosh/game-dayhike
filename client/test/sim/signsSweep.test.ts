...
describe("sign posts on real worlds", { timeout: 120_000 }, () => {
  it("names the summit on some arm, keeps every post off the bed, and emits each post once as a prop", () => {
    for (const seed of PROBE_SEEDS) {
      const { graph } = bowlFor(seed);
      const crest = graph.nodes[graph.summit]!;
      const posts = signPosts(graph, [{ name: "the summit", x: crest.x, z: crest.z }]);
      ...
      if (junctions > 0) {
        ...
        expect(named.has("the summit"), `seed ${seed}`).toBe(true);
      }
