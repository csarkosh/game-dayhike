...
/** One non-exempt edge pair closer than the gap: its two edge indices and
 * their actual (non-squared) segment distance. */
export type CloseEdgePair = { i: number; j: number; dist: number };

/**
 * Every non-adjacent-by-walking edge pair of `graph` whose segments come
 * closer than `gap`, with their actual distance. Two edges are exempt when
 * their nearest endpoints are less than `gap` of walking distance apart —
 * the trail's own measure of "the same junction", not node identity (two
 * edges of one junction can sit on nodes that never coincide, yet
 * legitimately run closer than `gap`). Copied from `trailBuild.test.ts`'s
 * "holds every edge's profile under the hard cap and every non-adjacent
 * pair TRAIL_EDGE_MIN_GAP apart" test so the exemption rule stays identical
 * on the synthetic frames and the 227-seed sweep.
 */
export function closeNonAdjacentEdgePairs(graph: TrailGraph, gap: number): CloseEdgePair[] {
  const within = new Map<number, Map<number, number>>();
  for (let n = 0; n < graph.nodes.length; n++) within.set(n, near(graph, n, gap));
  const close: CloseEdgePair[] = [];
  for (let i = 0; i < graph.edges.length; i++) {
    for (let j = i + 1; j < graph.edges.length; j++) {
      ...
      const distSq = segmentSegmentDistanceSq(a.x, a.z, b.x, b.z, c.x, c.z, d.x, d.z);
      if (distSq < gap * gap - 1e-6) close.push({ i, j, dist: Math.sqrt(distSq) });
    }
  }
  return close;
}
